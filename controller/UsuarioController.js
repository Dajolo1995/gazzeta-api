 ...
 //           const reg=await models.Usuario.find({'nombre':new regExp(valor,'i')},{'email': new regExp(valor,'i')})
    ...
    update: async (req,res,next) => {
        try {
            ...
            const reg = await models.Usuario.findByIdAndUpdate({_id:req.body._id},{nombre:req.body.nombre, email:req.body.email, password:req.body.password, rol:req.body.rol, estado: req.body.estado});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message:'Ocurrió un error'
